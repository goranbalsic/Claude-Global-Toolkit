...
| Path | Purpose |
|---|---|
| `commands/ctk/` | Slash commands: `/ctk:resume`, `/ctk:checkpoint`, `/ctk:decide`, `/ctk:plan`, `/ctk:verify`, `/ctk:review`, `/ctk:ship`. |
| `agents/` | Isolated investigators, reviewers, and verifier agents that return compact evidence digests. |
| `skills/` | Progressive-disclosure guidance for bounded continuity, evidence, and safe changes. |
