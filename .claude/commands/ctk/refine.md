---
description: "Propose one reviewable, evidence-based improvement to a project-local skill, checklist, or routing rule; apply only after explicit approval."
...
6. After an approved edit, record one line through the existing bounded
   history mechanism instead of creating a new log file:

   ```sh
   if [ -x "$CLAUDE_PROJECT_DIR/bin/ctk" ]; then
     "$CLAUDE_PROJECT_DIR/bin/ctk" state add "refine: <file> - <one-line summary>" --yes
   else
